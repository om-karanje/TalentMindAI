...
def calculate_offer_acceptance_score(
    ...
    return max(
        0.0,
        min(1.0, offer_acceptance_rate)
    )

# NOTICE PERIOD SCORE
def calculate_notice_period_score(
    notice_period_days
):
...
# OPEN TO WORK SCORE
def calculate_open_to_work_score(
    open_to_work_flag
):
    return 1.0 if open_to_work_flag else 0.5

# MARKET INTEREST SCORE
def calculate_market_interest_score(
    signals
):
    """Recruiter demand.
    Uses:
    ...
    profile_views_norm = normalize(
        profile_views,
        0,
        500
    )

    recruiter_saves_norm = normalize(
        recruiter_saves,
        0,
        50
    )

    search_appearance_norm = normalize(
        search_appearance,
        0,
        1000
    )
    ...
    market_score = (
        0.25 * profile_views_norm +
        0.40 * recruiter_saves_norm +
        0.35 * search_appearance_norm
    )
...
def calculate_behavior_score(
    ...
    profile_completeness = normalize(
        safe_float(
            signals.get(
    ...
    offer_acceptance_score = (
        calculate_offer_acceptance_score(
            signals.get(
                "offer_acceptance_rate",
                -1
            )
    ...
    behavior_score = (
        ...
        0.25 * interview_completion_rate +
...
def calculate_availability_score(
    ...
    availability_score = (
        0.50 * notice_period_score +
        0.30 * open_to_work_score +
        0.20 * activity_score
    )
...
def generate_behavior_features(
    ...
    market_interest_score = (
        calculate_market_interest_score(
            signals
        )
    )
